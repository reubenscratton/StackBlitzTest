define(function(require) {
  ...
  function keyCode(evt) {
    ...
    switch (evt.location) {
      default:
        ...
        switch (ret) {
          case keyCodes.SHIFT:
            return lastShiftLocation === 1
              ? keyCodes.SHIFT_LEFT
              : keyCodes.SHIFT_RIGHT;
          case keyCodes.ALT:
            return lastAltLocation === 1
              ? keyCodes.ALT_LEFT
              : keyCodes.ALT_RIGHT;
          case keyCodes.CTRL:
            return lastCtrlLocation === 1
              ? keyCodes.CTRL_LEFT
              : keyCodes.CTRL_RIGHT;
        }
        break;
  ...
  function keyUp(evt) {
    ...
    var code = keyCode(evt);
    if (cpu && cpu.sysvia) cpu.sysvia.keyUp(code);
    if (!running) return;
    /*if (evt.altKey) {
            var handler = emuKeyHandlers[code];
            if (handler) {
  ...
  Emulator.prototype.init = function() {
    ...
    if (audioContext) {
      ...
      // Safari...
      this.soundChip.jsAudioNode = audioContext.createScriptProcessor(
        2048,
        0,
        1
      );
      this.soundChip.jsAudioNode.onaudioprocess = _.bind(function pumpAudio(
        event
      ) {
    ...
    this.ready = false;
    Promise.all([cpu.initialise(), this.ddNoise.initialise()]).then(
      _.bind(function() {
        this.ready = true;
      }, this)
    );
  ...
  Emulator.prototype.sendRawKeyboardToBBC = function() {
    ...
    var sendCharHook = cpu.debugInstruction.add(
      _.bind(function nextCharHook() {
        var millis =
          cpu.cycleSeconds * 1000 +
          cpu.currentCycles / (ClocksPerSecond / 1000);
